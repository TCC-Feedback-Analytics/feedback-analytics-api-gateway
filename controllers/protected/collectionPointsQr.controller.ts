...
import { sendTypedError } from '../../lib/sendTypedError.js';
import { getCatalogQuestionsSnapshot } from '../../repositories/collectionPointsQr.repository.js';
import type { CatalogKind, QuestionOrder, SubquestionOrder } from '../../repositories/collectionPointsQr.repository.js';
import type { CatalogQuestionInput, CatalogSubquestionInput } from '../../types/catalogQuestion.types.js';
...
export async function getQrStatusController(req: Request, res: Response) {
  ...
  if (enterpriseError || !enterprise) {
    return sendTypedError(res, 404, API_ERROR_ENTERPRISE_NOT_FOUND, { active: false });
  }
...
export async function enableQrController(req: Request, res: Response) {
  ...
  if (anyCP) {
    ...
    if (updErr) return sendTypedError(res, 500, API_ERROR_UNABLE_TO_ACTIVATE_QR);
...
export async function disableQrController(req: Request, res: Response) {
  ...
  if (updErr) return sendTypedError(res, 500, API_ERROR_UNABLE_TO_DISABLE_QR);
...
export async function getQrCatalogController(req: Request, res: Response) {
  ...
  if (itemsError) return sendTypedError(res, 500, API_ERROR_COLLECTION_POINT_ERROR);
  ...
  if (pointsError) return sendTypedError(res, 500, API_ERROR_COLLECTION_POINT_ERROR);

  const pointByCatalog = new Map<string, { id: string; status: string }>();
  for (const point of points ?? []) {
    if (!point.catalog_item_id || pointByCatalog.has(point.catalog_item_id)) continue;
    pointByCatalog.set(point.catalog_item_id, { id: point.id, status: point.status });
  }
  ...
  if (questionsSnapshot.error) return sendTypedError(res, 500, API_ERROR_COLLECTION_POINT_ERROR);
...
export async function upsertCatalogQuestionsController(req: Request, res: Response) {
  ...
  if (enterpriseError || !enterprise) return sendTypedError(res, 404, API_ERROR_ENTERPRISE_NOT_FOUND);
  ...
  const questionByOrder = new Map<
    QuestionOrder,
    {
      ...
      is_active: boolean;
      subquestionsByOrder: Map<SubquestionOrder, { subquestion_order: SubquestionOrder; subquestion_text: string; is_active: boolean }>;
    }
  >();
  ...
  for (let index = 0; index < rawQuestions.length; index += 1) {
    ...
    const subquestionsByOrder = new Map<SubquestionOrder, { subquestion_order: SubquestionOrder; subquestion_text: string; is_active: boolean }>();
  ...
  for (const question of orderedQuestions) {
    ...
    if (updateQuestionError) return sendTypedError(res, 500, API_ERROR_UPDATE_FAILED);
    ...
    if (!questionId) {
      ...
      if (insertQuestionError || !insertedQuestion) return sendTypedError(res, 500, API_ERROR_UPDATE_FAILED);
    ...
    for (const subquestionOrder of [1, 2, 3] as SubquestionOrder[]) {
      ...
      const { data: updatedSubquestionRows, error: updateSubquestionError } = await supabase
        .from('feedback_question_subquestions')
        .update({
      ...
      if (updateSubquestionError) return sendTypedError(res, 500, API_ERROR_UPDATE_FAILED);
      if ((updatedSubquestionRows?.length ?? 0) > 0) continue;
  ...
  if (questionsSnapshot.error) return sendTypedError(res, 500, API_ERROR_UPDATE_FAILED);
...
export async function enableCatalogQrController(req: Request, res: Response) {
  ...
  if (catalogItemError || !catalogItem) return sendTypedError(res, 404, API_ERROR_COLLECTION_POINT_ERROR);
  ...
  if (activeCP) {
    return res.json({ catalog_item_id: catalogItem.id, collection_point_id: activeCP.id, active: true });
  }
  ...
  if (anyCP) {
    ...
    if (updErr) return sendTypedError(res, 500, API_ERROR_UNABLE_TO_ACTIVATE_QR);
    return res.json({ catalog_item_id: catalogItem.id, collection_point_id: anyCP.id, active: true });
  }
  ...
  if (createErr || !newCP) return sendTypedError(res, 500, API_ERROR_UNABLE_TO_CREATE_QR_CP);

  return res.json({ catalog_item_id: catalogItem.id, collection_point_id: newCP.id, active: true });
}
