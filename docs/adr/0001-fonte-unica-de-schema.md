...
- **Governança das tabelas Better Auth:** **verificado** — hoje são criadas *só* pelo `db/cutover/betterauth-enable.sql` manual (não há `@better-auth/cli` nas deps, ficam fora do `drizzle-kit`, e o `deploy-api.yml` não roda migration), com **3 definições mantidas à mão**. Decidir na Fase 2 se passam a ser geridas via `drizzle.config` (recomendado) ou pelo Better Auth CLI com o Drizzle só as descrevendo — evitar dois donos da mesma DDL.
...
- [x] Documentar/verificar o mecanismo real de migração das tabelas Better Auth em produção e incorporá-lo ao checklist/CI. **Verificado (2026-07-08):** são criadas *só* pelo `db/cutover/betterauth-enable.sql` manual — **sem** `@better-auth/cli` (ausente das deps), **fora** do `drizzle-kit` e o `deploy-api.yml` **não** roda migration; o `drizzleAdapter` só as consome em runtime. As 4 tabelas têm **3 definições à mão** (`src/auth/schema.ts` + `db/cutover/betterauth-enable.sql` + `db/schema/tables/public.better_auth.sql`), hoje coincidentes. Documentado em `db/cutover/README.md` e no checklist do `PULL_REQUEST_TEMPLATE.md`.
